...
<div class="w-full h-full flex flex-col items-center">
    <div class="flex justify-center w-full h-full">
        <div class="bg-slate-500 rounded-lg border-2 w-full h-full">
            {#if curTestInfo}
            <div class="flex flex-col">
                <div class="flex justify-between">
                        <h1 class="m-4 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">{curTestInfo.label}</h1>
                    </div>
                </div>
                <div class="flex h-4/5 w-full flex-col items-center justify-center overflow-hidden">
                    <div class="flex h-4/5 w-8/12 flex-col items-center justify-center">
                        ...
                        <div class="mb-6">
                            <div class="mb-6">
                                <Label for='age' color='gray' class='block mb-2'>Dein Alter</Label>
                                <Input id='age' color='base' placeholder="21" bind:value={userAge} />
                                {#if error}
                                <Helper class='mt-2' color='red'><span class="font-medium">Fehler</span> Bitte gebe eine gültiges Alter ein</Helper> 
                                {/if}
                            </div>
                        </div>
                    <div>
            ...
            {:else}
                <h1 class="m-4 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">Loading...</h1>
            {/if}
        </div>
    </div>
</div>