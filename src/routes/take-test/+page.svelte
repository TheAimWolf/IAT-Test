<script lang="ts">
	...
	async function showResults(testResults: testEntry[]) {
		...
		if (dScore === -1000) {
			testResultString =
				'Leider hast du zu viele Fehleingaben gemacht, weswegen der Test nicht ausgewertet werden kann.';
			return;
		}
		if (dScore === -1010) {
			testResultString =
				'Du bist leider zu schnell gewesen. Schonmal über eine E-Sports Karriere nachgedacht?';
			return;
		}
		fetch('/save', {
			method: 'PUT',
		...
		if (dScore <= -1) {
			testResultString = testInfo['b-full'];
		} else if (dScore > -1 && dScore <= -0.25) {
			testResultString = testInfo['b-half'];
		} else if (dScore > -0.25 && dScore < 0.25) {
			testResultString = testInfo['a-b'];
		} else if (dScore >= 0.25 && dScore < 1) {
			testResultString = testInfo['a-half'];
		} else if (dScore >= 1) {
			testResultString = testInfo['a-full'];
		}
	}
	...
	function calculateDScore(entries: testEntry[]): number {
		...
		if (validEntries.length / entries.length < 0.75) {
			return -1000;
		} //Error code for to many wrong tries
		let entriesLessThenMin = entries.filter((e) => e.time < 300);
		if (entriesLessThenMin.length / entries.length > 0.1) {
			return -1010;
		} //Error code for to fast answers
	...
	function getMousePosition(event: MouseEvent) {
		if (curState == 'test') {
			...
			if (!isButton && mousePosition < screenWidth / 2) {
				goLeft();
			} else if (!isButton) {
				goRight();
			}
...
<main>
	<div class="flex h-screen w-screen flex-col items-center justify-center lg:p-24">
		{#if curState == 'pretest'}
